...
class ReLU(FunctionNode):
    ...
    def _backward(gradient, *inputs):
        ...
        assert gradient.shape == inputs[0].shape
        "*** YOUR CODE HERE (Q3) ***"
        #apply the ReLU function element-wise on x.
...
class SquareLoss(FunctionNode):
    ...
    def _backward(gradient, *inputs):
        ...
        b = inputs[1]
        size = a.size
        down_grad_a = gradient*(a-b)/size
        down_grad_b = gradient*(b-a)/size
        return [down_grad_a,down_grad_b]
...
class SoftmaxLoss(FunctionNode):
    ...
    def _backward(gradient, *inputs):
        ...
        #dl/dy_i = -log(p_i) = -log_probs

        downstream_grad_for_logits = gradient*(np.exp(log_probs)-b)/size
        downstream_grad_for_labels = gradient*(-log_probs)/size
        
        return[downstream_grad_for_logits,downstream_grad_for_labels]
