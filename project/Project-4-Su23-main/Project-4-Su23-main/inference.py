...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        0. Query : P(Q|E_1 = e_1, ... E_k = e_k)

        1. start with initial factors ==> local CPTs (but instantiated by evidence)
        2. while there are still hidden variables (not Q orr evidence):
            pick a hidden variable H
            join all factors mentioning H
            Eliminate H
        3. join all the remaining factors
        4. normalize
        ...
        #for each v in order(vars), ...
        for v in eliminationOrder:
            #make-factor(v, e)
            currentFactorsList, joinFactor = joinFactorsByVariable(currentFactorsList, v)
            #only one unconditioned variable: no eliminate
            if len(joinFactor.unconditionedVariables())==1:
                continue
            if len(joinFactor.unconditionedVariables())>1:
                elim = eliminate(joinFactor,v)
                currentFactorsList.append(elim)
        #join all the remaining factors
        factors = joinFactors(currentFactorsList)
        #normalize
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for i in range(len(items)):
            only_item.append(items[i][1])
        return random.choices(keys,weights=only_item)[0]
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for i in allpos:
            self.beliefs[i] = self.beliefs[i]*self.getObservationProb(observation, pacpos, i, jailpos)
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        save = DiscreteDistribution()
        for oldPos in allpos:
            #oldPos refers to the previous ghost position
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            #newPosDist ==> (position x,position y): probability
            keys = list(newPosDist.keys()) #(position x, position y)
            values = list(newPosDist.values()) #probability
            for i in range(len(values)):
                save[keys[i]] += self.beliefs[oldPos]*values[i]
        self.beliefs = save

        #oldPos refers to the previous ghost position.
        #getPositionDistribution: P(G_t+1|gameState, G_t)

        #For each postion p in self.allPositions, 
        #newPosDist[p] is the probability
        #that the ghost is position p at time t+1,
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        chunk = self.numParticles // legal_pos_len
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for p in particles: #positions
            updated_beliefs[p] += self.getObservationProb(observation, pacpos, p, jailpos)        
        ...
        sum = 0
        values = list(updated_beliefs.values())
        for i in values:
            sum+=i
        if sum == 0:
            self.initializeUniformly(gameState) #in question 9
            updated_beliefs = self.getBeliefDistribution()

        self.particles = []
        for i in range(self.numParticles):
            self.particles.append(updated_beliefs.sample())
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        allparticle = self.particles
        updated_beliefs = DiscreteDistribution()
            

        for oldPos in allparticle:
            #oldPos refers to the previous ghost position
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            #newPosDist ==> (position x,position y): probability
            keys = list(newPosDist.keys()) #(position x, position y)
            values = list(newPosDist.values()) #probability
            for i in range(len(values)):
                #updated_beliefs[keys[i]] += self.beliefs[oldPos]*values[i]
                updated_beliefs[keys[i]] += values[i] # probability : self.beliefs -> number of particles
