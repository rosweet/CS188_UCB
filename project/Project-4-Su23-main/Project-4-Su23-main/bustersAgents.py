...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        #livingGhostPositionDistributions: a list of DiscreteDistribution objects
        #representing the position belief distributions for each of the ghosts that are still uncaptured
        #(position x, position y): probability

        "*** YOUR CODE HERE ***"

        ghost_number = len(livingGhostPositionDistributions)
        save = []

        for i in range(ghost_number):
            position_and_prob = livingGhostPositionDistributions[i]

            keys = list(position_and_prob.keys())
            values = list(position_and_prob.values())
        ...
        closest_distance = 10e100
        closest_ghost_position = None
        for i in save:
            candidate_distance = self.distancer.getDistance(pacmanPosition, i)
            if candidate_distance < closest_distance:
                closest_distance = candidate_distance
                closest_ghost_position = i

        greedy_distance = 10e100
        choosed_action = None

        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            new_distance = self.distancer.getDistance(closest_ghost_position, successorPosition)
            if action != None and new_distance < greedy_distance:
